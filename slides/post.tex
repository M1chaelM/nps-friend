...
\newcommand{\node}{%
  \begingroup\normalfont
  \includegraphics[height=\fontcharht\font`\c]{art/node.eps}%
  \endgroup
}

\newcommand{\threenode}{%
  \begingroup\normalfont
  \includegraphics[height=\fontcharht\font`\B]{art/3nodetree.eps}%
  \endgroup
}
...
\begin{tabular}{llllll}
  \onslide{1-}{             &                                 & $\bid{B}$ & $\id{nil}\in\id{RTL}$           &                       &} 
  \onslide{1-}{\\ \cline{4-4}} 
  \onslide{1-}{$\bid{B}$  & $\id{nil}\in\id{RTL}$}
  &\onslide{1-}{$\bid{R2}$             & $\id{node}(\id{nil})\in\id{RT}$} & \onslide{1-}{$\id{nil}\in\id{RTL}$ & $\bid{B}$} 
  \onslide{1-}{\\ \cline{2-2} \cline{4-5}}
  \onslide{1-}{$\bid{R2}$ & $\id{node}(\id{nil})\in\id{RT}$} & \onslide{1-}{$\bid{R1}$ & \multicolumn{2}{l}{ $\id{cons}(\id{node}(\id{nil}),\id{nil})\in\id{RTL}$} &} 
  \onslide{1-}{\\ \cline{2-5}}
  \onslide{1-}{$\bid{R1}$ & \multicolumn{4}{l}{$\id{cons}(\id{node}(\id{nil}),\id{cons}(\id{node}(\id{nil}),\id{nil}))
      \in \id{RTL}$}} 
  \onslide{1-}{ \\ \cline{2-5}}
  \onslide{1-}{$\bid{R2}$ & \multicolumn{4}{l}{$\id{node}(\id{cons}(\id{node}(\id{nil}),\id{cons}(\id{node}(\id{nil}),\id{nil})))\in\id{RT}$}}
\end{tabular}
\end{center}
\twocolumn[
lfrprop={},
...
%\begin{figure}[h]
%\centering
%\includegraphics[width=2.5in, height=.75in,keepaspectratio=true]{art/3_node_tree.eps}
