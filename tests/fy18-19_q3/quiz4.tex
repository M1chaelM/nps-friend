...
\question[3] Convert to prenex conjunctive normal form:
\[
...
\begin{align*}
&(\exists x A(x) \imp \forall x B(x)) \imp \forall x (A(x) \imp B(x))\\
&\equiv (\exists x A(x) \imp \forall y B(y)) \imp \forall z (A(z) \imp B(z))\\
&\equiv  \ngg (\ngg \exists x A(x) \lor \forall y B(y)) \lor \forall z (\ngg A(z) \lor B(z))\\
&\equiv  (\exists x A(x) \land \ngg \forall y B(y)) \lor \forall z (\ngg A(z) \lor B(z))\\
...
\question Let $A$ and $B$ be two problems such that $A$ reduces to $B$. 
\begin{parts}
\part[2] Which of the following can you conclude (choose all that apply)?
\begin{checkboxes}
