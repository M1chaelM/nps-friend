...
\clearpage
\question Answer the questions that follow with respect to the automaton shown
below:
...
\part[2] Let $\hat{\delta}(s_0,w) = s_0$. Which of the following are possible
...
\begin{itemize}
\item The automaton halts in state $s_0$ iff its input ends with an even number of ones. 
\item The automaton halts in state $s_1$ iff its input ends with \fillin[an odd
number of ones][2.25in].
\item The automaton halts in state $s_2$ iff \fillin[its input ends with a zero] [3in].
\end{itemize}
\part[3]\label{part:inductionrule} Which of the following induction rules is the best choice for our proof?
\begin{choices}
\uplevel{
\choice 
	\(\begin{array}[t]{l}
...
\part[2] Briefly justify your answer to part (\ref{part:inductionrule}):
