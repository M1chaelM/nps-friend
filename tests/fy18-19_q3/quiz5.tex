...
\[\Diamond ( p \imp q) \imp (\Diamond p \imp \Diamond q)\]
...
\question[4] Define propositions $p$ and $q$ as follows:\\
...
Write a statement in linear temporal logic that uses $p$ and $q$ to express the
liveness property, stated ``If a process attempts to enter its critical section
it will eventually succeed.''
\vspace{20mm}

\question Consider the following transition function, $\delta$:
\begin{align*}
  \delta(s_0,c) = s_0,\;\;& \delta(s_0,d) = s_1 \\
  \delta(s_1,d) = s_0,\;\;& \delta(s_1,c) = s_1 
\end{align*}
\begin{parts}
...
\choice $d$
\CorrectChoice $ccdd$
\CorrectChoice $cccddcccdd$
\choice $ccdcddccccdcddcccccddd$
\end{oneparchoices}
\end{parts}
\end{questions}
\end{document}
