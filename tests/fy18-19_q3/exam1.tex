...
\to \Sigma$, where $c \in \Sigma$ and we define $f(x) = c$ if there is some $y$ in
$\Sigma^*$ such that $cy = x$. 
...
\begin{parts}
\part[4] Evaluate the implications below. Write $T$ if the implication is
...
in $l$ is $0$.
\subpart \tf[F] (T/F) If $l \in RTL$ has derivation height $4$, then it contains at
least four nodes.
...
\begin{subparts} 
\subpart[3] Prove this claim holds for $n=1$.
\vspace{25mm}
\subpart[2] Does the claim hold for $n=0$? Justify your answer.
\vspace{15mm}
\end{subparts} 
